...
def get_size_of_smallest_dir_to_allow_update(disk_size=70000000, update_size=30000000):
    space_required = (update_size + filesystem[PurePath("/")].get_size()) - disk_size
    best_candidate = float("inf")
    for dir in filesystem.values():
        size = dir.get_size()
        if best_candidate > size > space_required:
            best_candidate = size
...
def main(input):
    ...
    print(filesystem)
    return get_size_of_smallest_dir_to_allow_update()
